...
#include <stdio.h>		/* printf(3), FILE, sprintf3), fscanf(3),
						   NULL, fopen(3), fclose(3)				*/
#include <math.h>		/* cos(3), sin(3)							*/
#include "dim.h"		/* SSM parameters							*/
#include "transbin.h"	/* TOL										*/
#include<stdlib.h>
#define min(a,b)	((a)<(b)?(a):(b))
#define max(a,b)	((a)>(b)?(a):(b))

static float mask_thickness[NUM_CAM] = {0},
			  elwidth[NUM_CAM] = {0},
			  patwidth[NUM_CAM] = {0},
			  patgap[NUM_CAM] = {0},
...
static int npat[NUM_CAM] = {0}, nelem[NUM_CAM] = {0},
		   nwinrod[NUM_CAM] = {0},
		   maskpat[NUM_PATTERNS][NUM_MASK_ELEM+2] = { {0} };

/*
  Code to compute transmission to a detector plane bin
  of an ASTROSAT SSM camera.

  This version computes pure geometric shadow.  
  The mask plate (and its projected thickness) is considered
  fully opaque.  Partial transmission through the mask plate 
  due to non-zero mean free path of photons is not taken into 
  account.

  The bin on the detector window plane is specified by inputs:
        xbin_det_lo (lower edge of the bin in x), xbin_width, and
        ybin_det_lo (lower edge of the bin in y), ybin_width.
            All dimensions are in mm.

  Other inputs are:
        thetax, thetay (radians) - angles from camera normal in
            x and y directions, specifying the direction of the source
        icam = camera number [BOOM, SLANT1 or SLANT2]

  Outputs are:
        xbin_width, ybin_width: values modified if they exceed limits
        trans: fractional transmission through the coded mask into the bin
        ageom: geometric area of the bin in sq. mm

  x-direction is along the position sensitive anodes (coding direction)
  y-direction is across the anodes (cross-coding direction)

*/
void transbin(int icam, float xbin_det_lo, float *xbin_width, float ybin_det_lo, float *ybin_width, float thetax, float thetay, float *trans, float *ageom){
		...
		for(iel = iel_start; iel <= iel_end; ++iel){
			...
			*/
			if(maskpat[ipat1][iel] == 0){
				...
				*/
				if (iel > 0 && maskpat[ipat1][iel-1] == 1)
				...
				*/
				if (iel <= nelem[icam] && maskpat[ipat1][iel+1] == 1)
	...
	*/
	if (fpat2 > 0.0){
		...
		for(iel = iel_start; iel <= iel_end; ++iel){
			...
			if (maskpat[ipat2][iel] == 0){

				xel_lo = xref+iel*elwidth[icam];

				if(iel > 0 && maskpat[ipat2][iel-1] == 1)
					xel_lo = xel_lo + el_extn_lo;

				xel_hi = xref+(iel+1)*elwidth[icam];

				if (iel <= nelem[icam] && maskpat[ipat2][iel+1] == 1)
					xel_hi = xel_hi + el_extn_hi;
...
void static setmaskparams(){
	...
	*/
	...
	maskpat[ipat][0] = 0; /* pad closed elem in the beginning */
	maskpat[ipat][NUM_MASK_ELEM+1] = 0;     /* and at the end */
		
	for(ielem = 1; ielem <= NUM_MASK_ELEM; ++ielem)
		fscanf(fp, "%1d", &maskpat[ipat][ielem]);
...
void static setbinlims(){

	/*
	  Set the limit on maximum individual bin size that can be
	  processed by the algorithm being used.
	*/
...
void static setwinrod(){
	...
	*/
	for(icam = 1; icam <= NUM_CAM; ++icam){ /* we have three cameras */

		switch(icam){
			case 1: cam = BOOM;   break;
			case 2: cam = SLANT1; break;
			case 3: cam = SLANT2; break;
		}

		for(irod = 0; irod <= nwinrod[cam]-1; ++irod){
			ywinrod_0[cam][irod] = CELL_WD_Y*irod + DET_ORIGIN_Y;
			ywinrod_1[cam][irod] = 0.0;
			winrod_dia[cam][irod] = WIN_SUP_ROD_DIAMETER;
		}
	}
...
void static setcalwire(){

	/*
	  Mounting tilt is accounted for as linear coefficients of y, e.g.
	      height of the (axis of the) wire from the window plane:
		       zcalwire_0+zcalwire_1*y
	      x-position of the (axis of the) wire:
		       xcalwire_0+xcalwire_1*y
	  calwire_dia is the diameter of the calibration wire
	*/
...
void transbininit(){
	...
	setmaskparams();
	setbinlims(); /* SHOULD be called only after calling setmaskparams */
	setcalwire();
	setwinrod();
...
int main()
{
	...
	float icam=1;
	float i=0;
	float xbin_det_lo=0,ybin_det_lo=0,thetax=0,thetay=0;
	float xbin_width=1,ybin_width=1,trans,ageom;
	...
	ybin_width=12.0;
	ybin_det_lo=(iwire-1)*12.0-48.0;
	ibin=0;
	xbin_width=0.25;
		
	for(i=0;i<241;i++)
	{	
		xbin_det_lo = -30.0+ibin*xbin_width;
		ibin=ibin+1;
		
		transbin(icam,xbin_det_lo,&xbin_width,ybin_det_lo,&ybin_width,thetax,thetay,&trans,&ageom);
		printf("\t%f\t%f\n",ibin,trans);
